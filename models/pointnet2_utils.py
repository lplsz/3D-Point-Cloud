...
def square_distance(src, dst):
    """
    Distance of each point from the centroids during the ball query.
...
def index_points(points, idx):
    """
    Return the first S points of each of B samples. 
    ...
    B = points.shape[0]
    view_shape = list(idx.shape)                    # [B, N, 1]
    view_shape[1:] = [1] * (len(view_shape) - 1)    # [B, 1, 1]
    repeat_shape = list(idx.shape)
    repeat_shape[0] = 1                             # [1, N, 1]
    batch_indices = torch.arange(B, dtype=torch.long).to(device).view(view_shape).repeat(repeat_shape)  # [B, N, 1]
...
def query_ball_point(radius, nsample, xyz, new_xyz):
    ...
    Return:
        group_idx: grouped points index, [B, S, nsample]        Index of the points in the sphere
    """
    # TODO: Change here to get kNN
    device = xyz.device
    B, N, C = xyz.shape
    _, S, _ = new_xyz.shape
...
def sample_and_group(npoint, radius, nsample, xyz, points, returnfps=False):
    ...
    new_xyz = index_points(xyz, fps_idx)                        # Get the centroids
    idx = query_ball_point(radius, nsample, xyz, new_xyz)       # Get the centroids' neighbour
    grouped_xyz = index_points(xyz, idx)                        # Get those neighbour
    grouped_xyz_norm = grouped_xyz - new_xyz.view(B, S, 1, C)   # Align the points by letting the centroid be the origin (0, 0, 0)

    # If each points have additional features, concate them to have point of feature [3 + D]
    ...
    # Return the centroids (Coordination only) and the neighbours of each centroid (Coordination + Feature)


def sample_and_group_all(xyz, points):
    """
    Treat all points as a whole group, the centroid is just the origin

    B: Number of batch
    N: Batch size
    3: Dimension of the position data
    ...
    grouped_xyz = xyz.view(B, 1, N, C)              # Add a dimension of length 1
...
class PointNetSetAbstraction(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        for i, conv in enumerate(self.mlp_convs):
            bn = self.mlp_bns[i]                        # MLP on each point
            new_points =  F.relu(bn(conv(new_points)))

        new_points = torch.max(new_points, 2)[0]        # Max Pooling along the samples
...
class PointNetSetAbstractionMsg(nn.Module):
    '''
    Get the features at different radius (Density) and concatenate together
    '''
    ...
    def forward(self, xyz, points):
        ...
        Return:
            ...
            new_points_concat: sample points feature data, [B, D', S]
        """
        xyz = xyz.permute(0, 2, 1)              # Rearrange the dimensions (0, 1, 2) -> (0, 2, 1)
        if points is not None:
            points = points.permute(0, 2, 1)
...
class PointNetFeaturePropagation(nn.Module):
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        """
        Input:
            xyz1: input points position data, [B, C, N]             N points
            xyz2: sampled input points position data, [B, C, S]     S points
        ...
        dists, idx = dists.sort(dim=-1)                 # [B, N, S]: Sort the distance accendingly
        dists, idx = dists[:, :, :1], idx[:, :, :1]     # [B, N, 1]: Take the closest points
        
        interpolated_points = index_points(points2, torch.squeeze(idx, 2))   # [B, N, 1] -> [B, N]


        if points1 is not None:
            points1 = points1.permute(0, 2, 1)      # [B, N, D]
            new_points = torch.cat([points1, interpolated_points], dim=-1)  # [B, N, 2D]
        else:
